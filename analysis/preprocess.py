...
for provider in providers:
    ...
    for f in dirlist:
        ...
        if f.endswith('.m3u8'):
            ...
            chunk_targetduration.add(float(root.attrib['maxSegmentDuration'][2:-1]))

            for period in root.findall('mpd:Period', namespaces):
                #audio_codec = None
                for adaptationset in period.findall('mpd:AdaptationSet', namespaces):
                    adapt_attrs = adaptationset.attrib
                    if adapt_attrs['contentType'] == 'audio':
                        if 'codecs' in adapt_attrs:
                            codec_list.add(adapt_attrs['codecs'])
                        else:
                            for representation in adaptationset.findall('mpd:Representation', namespaces):
                                codec_list.add(representation.attrib['codecs'])
                    elif adapt_attrs['contentType'] == 'video':
                        if 'codecs' in adapt_attrs:
                            codec_list.add(adapt_attrs['codecs'])
                        
                        for representation in adaptationset.findall('mpd:Representation', namespaces):
                            rep_attrs = representation.attrib
                            bitrates.append(int(rep_attrs['bandwidth']))
                            resolutions.add(rep_attrs['width']+'x'+rep_attrs['height'])
                            if 'codecs' in rep_attrs:
                                codec_list.add(rep_attrs['codecs'])
                    
                    media = adaptationset.find('mpd:SegmentTemplate', namespaces).attrib['media']
                    url = media
                    ...
                    if not media.startswith('http'):
                        ...
                        for logrecord in loglines:
                            if '.mpd' in logrecord:
                                url = logrecord[:logrecord.find('.mpd')+4]
                                url = url[:url.rfind('/')+1] + media
                                break
